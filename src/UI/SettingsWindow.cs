...
public class SettingsWindow : Window
{
    ...
    private TabItem MakeSubFolderMenuTab()
    {
        ...
        MakeCheck(panel, "カスタムカラーの設定を使用する",    s.UseCustomColors, v => { s.UseCustomColors = v; _save(); });
        ...
        panel.Children.Add(MakeColorRow("背景色 (#AARRGGBB):",     s.BackgroundColor, v => { s.BackgroundColor = v; _save(); }));
        panel.Children.Add(MakeColorRow("フォルダ色 (#AARRGGBB):", s.FolderColor,     v => { s.FolderColor     = v; _save(); }));
        panel.Children.Add(MakeColorRow("ファイル色 (#AARRGGBB):", s.FileColor,       v => { s.FileColor       = v; _save(); }));
        panel.Children.Add(MakeColorRow("ハイライト色 (#AARRGGBB):", s.HighlightColor, v => { s.HighlightColor = v; _save(); }));
    ...
    private TabItem MakePreviewExtensionsTab()
    {
        ...
        fgBox.LostFocus += (_, _) =>
        {
            if (IsValidHexColor(fgBox.Text)) { p.TextForegroundColor = fgBox.Text.Trim(); _save(); }
        };
        ...
        bgBox.LostFocus += (_, _) =>
        {
            if (IsValidHexColor(bgBox.Text)) { p.TextBackgroundColor = bgBox.Text.Trim(); _save(); }
        };
