...
Windows 11 File Explorerのパス欄の下にタブバーを追加するツール。  
...
| **タブバー表示/非表示切替** | 設定画面の「一般」タブからタブバー自体の表示・非表示を切り替え可能（デフォルトON） |
...
- [ ] フォルダアイコンの色分け表示
- [ ] ブックマークバー、パス欄の下にフォルダへのショートカットを表示（クリックで Shell.Application 経由ナビゲート）
